...
func doRoomTransition(currentRoom, nextRoom, isTransitionSouth):
	...
	if nextRoom.modulate.a != 1:
		tween.interpolate_property(nextRoom, "modulate", Color(1,1,1,0), Color(1,1,1,1), 0.3, Tween.TRANS_LINEAR, Tween.TRANS_LINEAR)					
	#fade out current room, unless we're going south, in which case it's not in the way
	if !isTransitionSouth:
		tween.interpolate_property(currentRoom, "modulate", Color(1,1,1,1), Color(1,1,1,0), 0.3, Tween.TRANS_LINEAR, Tween.TRANS_LINEAR)	
	tween.start()
